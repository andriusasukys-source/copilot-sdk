...
use github_copilot_sdk::{
    ProviderConfig, ProviderEndpointType, ProviderEndpointWireApi, SessionConfig,
};
...
fn opt_in_env() -> (OsString, OsString) {
    (
        "COPILOT_ALLOW_GET_PROVIDER_ENDPOINT".into(),
        "true".into(),
    )
}
    ...
    with_e2e_context(
        ...
        |ctx| {
            Box::pin(async move {
                ...
                assert!(
                    matches!(endpoint.wire_api, Some(ProviderEndpointWireApi::Completions)),
