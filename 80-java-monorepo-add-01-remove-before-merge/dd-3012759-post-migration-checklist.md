...
| `java-deploy-site.yml` successfully deploys docs | ✅ | | Done in [1524](https://github.com/github/copilot-sdk/pull/1524) |
...
| `CODEOWNERS` includes `java/` path | ✅ | Only `* @github/copilot-sdk` — no Java-specific owners. Issue #89 marked done in plan. | WONTFIX. See [ghcp-sp-80](https://github.com/github/copilot-sdk-internal/issues/89) |
...
| `sdk-consistency-review` includes `java/` in path triggers | ✅️ |  |  |
...
| Java codegen integrated into `scripts/codegen/` | ✅ |  | WONTFIX |
...
| Item | Status | Notes | Human notes |
|------|--------|-------|-------------|
| `copilot-sdk-java` repo archived | ✅ |  | Continue to use this repo to host API docs https://github.github.com/copilot-sdk-java/ |
...
1. **E2E harness uses local paths** — Refactor `java/pom.xml` to use the local `test/harness/` and `test/snapshots/` instead of cloning the monorepo into `target/`. See issue [1580](https://github.com/github/copilot-sdk/issues/1580)
